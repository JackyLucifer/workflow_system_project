...
public:
     ...
     * @brief 构造函数
     *
     * @param maxMemoryLimit 最大内存限制（字节），0表示无限制
     *
     * 功能：初始化内存池，预分配第一批内存块
     *
     * @note 构造后会自动调用 expandPool() 分配初始内存
     */
    explicit MemoryPool(size_t maxMemoryLimit = 0)
        : freeList_(nullptr)
        , allocatedBlocks_(0)
        , totalMemoryBytes_(0)
     ...
     * 3. 返回该块的指针
     *
     * @return T* 指向内存块的指针，如果超过内存上限则返回 nullptr
     *
     * @note 线程安全
     * @note 返回的内存未初始化，需要使用 placement new 构造对象
     * @note 时间复杂度：O(1)
     * @note 如果设置了内存上限且超过限制，返回 nullptr
     *
     * 示例：
     * @code
     * MyData* data = pool.allocate();
    ...
    void clear() {
        std::lock_guard<std::mutex> lock(mutex_);
        // 删除每次 expandPool 分配的整块内存
        for (char* batch : memoryBatches_) {
            delete[] batch;
        }
        memoryBatches_.clear();
        freeList_ = nullptr;
        allocatedBlocks_ = 0;
        totalMemoryBytes_ = 0;
        allocationCount_ = 0;
    ...
    void defragment() {
        ...
        // 如果碎片不严重（空闲块 > 总分配块的50%），无需整理
        if (allocatedBlocks_ > 0 && freeCount > allocatedBlocks_ / 2) {
            return;
        }

        // 重建空闲列表（这里只是示例，实际实现可能更复杂）
        // 对于简单的链表结构，碎片整理效果有限
        // 更复杂的实现可能需要重新分配内存块
    }
     ...
     */
    struct MemoryStats {
        size_t allocatedBlocks;      // 已分配的块数
        size_t totalMemoryBytes;     // 总内存字节数
        size_t maxMemoryLimit;       // 最大内存限制
        size_t allocationCount;      // 分配次数计数
        size_t availableCount;       // 可用块数
    };
    ...
    MemoryStats getStats() const {
        ...
        stats.maxMemoryLimit = maxMemoryLimit_;
     ...
     * @brief 设置最大内存限制
     *
     * @param maxBytes 最大内存字节数，0表示无限制
     *
     * @note 线程安全
     */
    void setMaxMemoryLimit(size_t maxBytes) {
        std::lock_guard<std::mutex> lock(mutex_);
        maxMemoryLimit_ = maxBytes;
    }
...
private:
     ...
     *
     * 流程：
     * 1. 检查内存上限，如果超过限制则返回 false
     * 2. 计算需要分配的总内存大小
     * 3. 一次性分配 PoolSize 个 T 对象的内存
     * 4. 将每个块插入到空闲列表
     *
     * @return true 扩容成功，false 达到内存上限
     *
     * @note 只在类内部调用，用户不应直接调用
     * @note 时间复杂度：O(n)
     */
    bool expandPool() {
        const size_t blockSize = sizeof(T) > sizeof(Block) ? sizeof(T) : sizeof(Block);
        const size_t totalSize = PoolSize * blockSize;

        // 检查内存上限
        if (maxMemoryLimit_ > 0 && totalMemoryBytes_ + totalSize > maxMemoryLimit_) {
            return false;  // 达到内存上限，拒绝扩容
        }

        char* memory = new (std::nothrow) char[totalSize];
        if (!memory) {
            return false;  // 内存分配失败
        }

        memoryBatches_.push_back(memory);
        ...
        allocatedBlocks_++;
        totalMemoryBytes_ += totalSize;
        return true;
    }

    mutable std::mutex mutex_;          // 保护内存池的互斥锁
    Block* freeList_;                   // 空闲块链表头指针
    size_t allocatedBlocks_;            // 已分配的块数
    size_t totalMemoryBytes_;           // 总内存字节数
    size_t maxMemoryLimit_;             // 最大内存限制（0表示无限制）
    size_t allocationCount_;            // 分配次数计数
    std::vector<char*> memoryBatches_;  // 每次 expandPool 分配的内存批次
};
