...
public:
    WorkflowStateException(const std::string& message,
        ...
        , toState_(to) {
        std::ostringstream oss;
        oss << message << " (from " << stateToString(from)
           << " to " << stateToString(to) << ")";
        message_ = oss.str();
    }
 ...
 * 如果执行失败，记录错误并重新抛出异常
 */
#define TRY_WF_OR_THROW(context) \
    try

#define CATCH_WF_OR_THROW \
    catch (const WorkflowSystem::WorkflowException& e) { \
        WorkflowSystem::ExceptionHandler::handle(e, context); \
        throw; \
    } catch (const std::exception& e) { \
        ...
        WorkflowSystem::ExceptionHandler::handleUnknown(context); \
        throw std::runtime_error("Unknown exception in " context); \
    }
